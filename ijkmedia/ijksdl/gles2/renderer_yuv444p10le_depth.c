...
static GLboolean yuv444p10le_depth_use(IJK_GLES2_Renderer *renderer)
{
...
static GLsizei yuv444p10le_depth_getBufferWidth(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return 0;
...
static GLubyte* yuv444p10le_depth_getLuminanceDataPointer(GLsizei* outWidth, GLsizei* outHeight, GLsizei* outLength, bool* outIsCopied, SDL_VoutOverlay* overlay) {
    if (!overlay)
        return NULL;
...
static GLboolean yuv444p10le_depth_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!renderer || !overlay)
        return GL_FALSE;
    ...
    glBindTexture(GL_TEXTURE_2D, renderer->depth_texture);  IJK_GLES2_checkError_TRACE("glBindTexture");
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_LUMINANCE_ALPHA,
                 widths[0],
                 ...
                 GL_LUMINANCE_ALPHA,
                 GL_UNSIGNED_BYTE,
                 pixels[0]);    IJK_GLES2_checkError_TRACE("glTexImage2D");
...
{
    ALOGI("create render yuv444p10le_depth\n");
    IJK_GLES2_Renderer *renderer = IJK_GLES2_Renderer_create_base(IJK_GLES2_getFragmentShader_yuv444p10le_depth());
    if (!renderer)
        goto fail;
    ...
    renderer->func_use            = yuv444p10le_depth_use;
    renderer->func_getBufferWidth = yuv444p10le_depth_getBufferWidth;
    renderer->func_uploadTexture  = yuv444p10le_depth_uploadTexture;
    renderer->func_getLuminanceDataPointer = yuv444p10le_depth_getLuminanceDataPointer;
