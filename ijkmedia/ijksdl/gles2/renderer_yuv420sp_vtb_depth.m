...
static GLboolean yuv420sp_vtb_depth_use(IJK_GLES2_Renderer *renderer)
{
//    ALOGI("use render yuv420sp_vtb_depth\n");
...
static GLvoid yuv420sp_vtb_depth_clean_textures(IJK_GLES2_Renderer *renderer)
{
...
static GLsizei yuv420sp_vtb_depth_getBufferWidth(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return 0;
...
static GLubyte* yuv420sp_vtb_depth_getLuminanceDataPointer(GLsizei* outWidth, GLsizei* outHeight, GLsizei* outLength, bool* outIsCopied, SDL_VoutOverlay* overlay) {
    if (!overlay)
        return NULL;
...
static GLboolean yuv420sp_vtb_depth_uploadTexture(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    ...
    switch (overlay->format) {
        case SDL_FCC__VTB:
            break;
        default:
            ALOGE("[yuv420sp_vtb_depth] unexpected format %x\n", overlay->format);
            return GL_FALSE;
    }
    ...
    yuv420sp_vtb_depth_clean_textures(renderer);
...
static GLvoid yuv420sp_vtb_depth_destroy(IJK_GLES2_Renderer *renderer)
{
    if (!renderer || !renderer->opaque)
        return;
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_yuv420sp_vtb_depth(SDL_VoutOverlay *overlay)
{
    ...
    if (!overlay) {
        ...
        return IJK_GLES2_Renderer_create_yuv420sp_depth();
    }
    ...
    ALOGI("create render yuv420sp_vtb_depth\n");
    IJK_GLES2_Renderer *renderer = IJK_GLES2_Renderer_create_base(IJK_GLES2_getFragmentShader_yuv420sp_depth());
    if (!renderer)
        goto fail;
    ...
    renderer->func_use            = yuv420sp_vtb_depth_use;
    renderer->func_getBufferWidth = yuv420sp_vtb_depth_getBufferWidth;
    renderer->func_uploadTexture  = yuv420sp_vtb_depth_uploadTexture;
    renderer->func_getLuminanceDataPointer = yuv420sp_vtb_depth_getLuminanceDataPointer;
    renderer->func_destroy        = yuv420sp_vtb_depth_destroy;
